...
| Artifact registry | `.pgmcp/config/artifacts.yaml` contains `artifact_types: []`, while template configuration exists. | Design must identify the authoritative artifact-discovery model before changing permissions. |
...
| Cross-config contracts | `issues.yaml`, `workphases.yaml`, `artifacts.yaml` | Required issue categories, dynamic phase validation, artifact/path permissions |
...
- Does issue #2 include `policies.yaml`, test-template output paths, affected project coding-standard references, and the empty artifact registry, or should any become separate issues?
- Which initial Ruff, mypy, and Pyright scopes belong here versus the separate quality/environment workstream?
...
- How is artifact discovery authoritative while `artifacts.yaml` is empty?
